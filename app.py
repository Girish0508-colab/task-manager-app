...
DATABASE = "/tmp/tasks.db" if os.getenv("RAILWAY_ENV") else os.path.join(BASE_DIR, "tasks.db")
...
def add():
    ...
    if "exam" in text or "deadline" in text or "urgent" in text:
        ...
        priority = "Low"
    conn = get_db()
    conn.execute("INSERT INTO tasks(title,deadline,priority,status,user_id) VALUES(?,?,?,?,?)",
